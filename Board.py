...
class Board(object):
    def __init__(self, room_names, character_names, players):
        ...
        self.vertices = room_vertices + start_vertices + blank_vertices
    ...
    def create_adjacency_matrix(self):
        ...
        ad_matrix = numpy.zeros((num_v, num_v))
    ...
    def get_can_reach(self, player_i, step):
        ad_matrix = self.create_adjacency_matrix()
        player_curr_index = self.get_v_index(self.players[player_i].curr_location)
        can_reach = ad_matrix.getrow(player_curr_index)
        print(can_reach)
        if step > 1:
            for i in range(step - 1):
                can_reach = can_reach.dot(ad_matrix)
